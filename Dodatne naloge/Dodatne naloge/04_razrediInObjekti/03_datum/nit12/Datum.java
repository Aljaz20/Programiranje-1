public class Datum{
    ...
    public int razlika(Datum datum){
        int stdni = 0;
        int d = 0;
        stdni = (this.Leto - datum.Leto)*365;
        if(datum.jePred(this)){
            for(int i = datum.Leto; i < this.Leto -1; i++){
                if(i % 400 == 0 || (i % 4 == 0 && i % 100 != 0)){
                    stdni++;
                }
                d=1;
            }
            datum = new Datum(datum.Dan, datum.Mesec, this.Leto - d);
            while(!datum.jeEnakKot(this)){
                stdni++;
                datum = datum.naslednik();
            }
        }else{
            for(int i = this.Leto; i < datum.Leto - 1; i++){
                if(i % 400 == 0 || (i % 4 == 0 && i % 100 != 0)){
                    stdni--;
                }
                d=1;
